...
// Set some content to print
    if($mFrom != "" && $mTo != ""){
        ...
        $get_name = $rcp->GetApproverName();
        while ($row = $get_name->fetch(PDO::FETCH_ASSOC)) {
            extract($row);
            $mApprvr[] =$row['approver_name'];
        }

        $data = $rcp->GetAllDeptInRcpAdminFromAndTo();
        ...
        $data = $rcp->GetAllDeptInRcpAdminFromOnly();
        while ($row = $data->fetch(PDO::FETCH_ASSOC)) {
        extract($row);
        $hasValue = true;
        $tbl .= '
        ...
        $tbl = '
            ...
            <tr nobr="true">
             ...
             <tr nobr="true">
                <th colspan="5" style="font-size: 10px">Reports of <strong>'.$dept_name.'</strong></th>
             </tr>
             <tr nobr="true" style="font-size: 11px; font-weight: bold;" align="center">
                <th style=" border-top: 1px solid black; border-right: 1px solid black; border-left: 1px solid black; border-bottom: 1px solid black">RCP No.</th>
                <th style=" border-top: 1px solid black; border-right: 1px solid black; border-bottom: 1px solid black">Requestor</th>
