...
$rcp->user_id = $prmy_id;
$get_username = $rcp->GetRqstrName();
while ($row = $get_username->fetch(PDO::FETCH_ASSOC)) {
    extract($row);
    $prmy_name = $row['approver_name'];
}
...
$pdf->SetTitle('Requestor Reports');
...
// Set some content to print
    if($mFrom != "" && $mTo != ""){
        ...
        <table  cellpadding="4">
        <tr nobr="true">
            <th colspan="6"><strong>'.$prmy_name.'</strong></th>
         ...
         ';
        $rcp->rcp_approver_id = $prmy_id;
        $rcp->startDate = $mFrom;
        $rcp->endDate = $mTo;
        $hasValue = false;
        $data = $rcp->GetAllPrmyApprvrAdminFromAndTo();
        ...
        $rcp->startDate = $mFrom;
        $hasValue = false;
        $data = $rcp->GetAllPrmyApprvrAdminFromOnly();
        while ($row = $data->fetch(PDO::FETCH_ASSOC)) {
        extract($row);
        $hasValue = true;
        $tbl .= '
        ...
        <tr nobr="true">
         ...
         <tr nobr="true">
            <th colspan="5" style="font-size: 10px">Reports from <strong>'.$prmy_name.'</strong></th>
         </tr>
         <tr nobr="true" style="font-size: 10px; font-weight: bold;" align="center">
            <th style=" border-top: 1px solid black; border-right: 1px solid black; border-left: 1px solid black; border-bottom: 1px solid black">RCP No.</th>
            <th style=" border-top: 1px solid black; border-right: 1px solid black; border-bottom: 1px solid black">Requestor</th>
        ...
        $rcp->rcp_approver_id = $prmy_id;
        $hasValue = false;
        $data = $rcp->GetAllPrmyApprvrAdmin();
        while ($row = $data->fetch(PDO::FETCH_ASSOC)) {
        extract($row);
        $hasValue = true;
        $tbl .= '
