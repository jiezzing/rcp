<?php
	class Select
	{
        ...
        {
            $query = "SELECT * FROM rcp_file rcp, user_file usr, project_file, company_file, department_file WHERE rcp_project=proj_code AND rcp_company=comp_code AND rcp_department=dept_code AND rcp.rcp_status = 'Pending' AND rcp.rcp_employee_id=? AND rcp.rcp_approver_id=usr.user_id ORDER BY rcp.created_at DESC";
			$this->conn->setAttribute( PDO::ATTR_ERRMODE, PDO:: ERRMODE_WARNING);
			$sel = $this->conn->prepare($query);
